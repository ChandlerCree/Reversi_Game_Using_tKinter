...
class LoginView():
    ...
    def main_screen(self):
        ...
        tk.Label(text="", height="2", bg=bg_1, font=("Calibri", 6)).pack()
        tk.Button(text="Guest Login", height="1", width="12", command=self.logged_as_guest, bg=bg_2, fg=fg_1, font=("Calibri", 18, "bold"), activebackground=cor_1).pack()
    ...
    def login(self):
        ...
        tk.Label(self.login_screen, text="REVERSI", bg=bg_2, width="300", height="1" , font=("Calibri", 24, "bold"), fg=fg_1, borderwidth=4, relief="groove", pady="10").pack()
        tk.Label(self.login_screen, text="", height="2", bg=bg_1, font=("Calibri", 2)).pack()
        tk.Label(self.login_screen, text="Please enter details below to login", font=("Calibri", 12), fg=fg_1, bg=bg_1).pack()
        tk.Label(self.login_screen, text="", height="2", bg=bg_1, font=("Calibri", 2)).pack()
        ...
        self.username_entry_login = tk.Entry(self.login_screen, textvariable=self.username_verify_login, bg=bg_2, fg=fg_1, font=("Calibri", 18, "bold"))
        self.username_entry_login.pack()
        tk.Label(self.login_screen, text="Password * ", font=("Calibri", 12), fg=fg_1, bg=bg_1).pack()
        self.password_entry_login = tk.Entry(self.login_screen, textvariable=self.password_verify_login, bg=bg_2, show = "*", fg=fg_1, font=("Calibri", 18, "bold"))
        self.password_entry_login.pack()

        tk.Label(self.login_screen, text="", bg=bg_1).pack()
        tk.Button(self.login_screen, text="Login", width=12, height=1, fg=fg_1, bg=bg_2, font=("Calibri", 18, "bold"), command=lambda:[self.remove_verify_text(), self.login_verify()]).pack()
    ...
    def login_verify(self):
        ...
        if user_temp == 1:
            ...
            if self.password_login == pass_temp:
                ...
                self.result_label = tk.Label(self.login_screen, font=("Calibri", 12), fg='red', bg=bg_1, text="Password not recognized")
                self.result_label.pack()
        else:
            self.result_label = tk.Label(self.login_screen, font=("Calibri", 12), fg='red', bg=bg_1, text="User not found")
            self.result_label.pack()
    ...
    def register(self):
        ...
        tk.Label(self.register_screen, text="REVERSI", bg=bg_2, width="300", height="1" , font=("Calibri", 24, "bold"), fg=fg_1, borderwidth=4, relief="groove", pady="10").pack()
        ...
        self.username_entry_register = tk.Entry(self.register_screen, textvariable=self.username_register, bg=bg_2, fg=fg_1, font=("Calibri", 18, "bold"))
        self.username_entry_register.pack()
        tk.Label(self.register_screen, text="Password * ", font=("Calibri", 12), fg=fg_1, bg=bg_1).pack()
        self.password_entry_register = tk.Entry(self.register_screen, textvariable=self.password_register, bg=bg_2, show = "*", fg=fg_1, font=("Calibri", 18, "bold"))
        self.password_entry_register.pack()

        tk.Label(self.register_screen, text="", bg=bg_1).pack()
        tk.Button(self.register_screen, text="Register", width=12, height=1, fg=fg_1, bg=bg_2, font=("Calibri", 18, "bold"), command=self.register_user).pack()
