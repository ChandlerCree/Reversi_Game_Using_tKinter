...
from controller.online_game_manager import OnlineGameManager
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.username_label = tk.Label(self.frame, text="Welcome {}!\nCurrent Elo: {}\nMatches Played: {}".format(self.default_username, str(self.default_elo), str(self.matches_played)), 
        ...
        self.new_ai_game_button = tk.Button(self.frame, text="AI Game", height="1", width="14", command=self.new_ai_game,
                                            bg=bg_2, fg=fg_1, font=("Calibri", 18, "bold"), activebackground=cor_1, state="disabled")
        self.new_ai_game_button.grid(row=2, column=0, padx=(8, 4), pady=4)

        self.new_online_game_button = tk.Button(self.frame, text="Online Game", height="1", width="14", command=self.new_online_game,
                                            bg=bg_2, fg=fg_1, font=("Calibri", 18, "bold"), activebackground=cor_1, state="normal")
    ...
    def change_username_label(self):
        self.username_label.configure(text="Welcome {}!\nCurrent Elo: {}\nMatches Played: {}".format(self.logged_user, self.logged_elo, self.matches_played))
    ...
    def new_online_game(self):
        
        game = OnlineGame(size=int(self.board_size))
        game_win = GUIView(self.master, game.board)

        game_win.p1 = self.p1
        game_win.p2 = self.p2

        controller = OnlineGameManager(game, game_win, self.user_logged_in)
        controller.run_game()
