...
class GUIView(tk.Frame, GameView):
    ...
    def makeBoard(self):
        ...
        self.player1Label = tk.Label(self, text="Player 1's score is: 2", padx=10, pady=10, font=("Calibri", 12, "bold"), bg=bg_1, fg=fg_1)
        self.player1Label.grid(row=1, columnspan=self.boardSize, sticky='ew')

        self.player2Label = tk.Label(self, text="Player 2's score is: 2", padx=10, pady=10, font=("Calibri", 12, "bold"), bg=bg_1, fg=fg_1)
        self.player2Label.grid(row=self.boardSize*3, columnspan=self.boardSize, sticky='ew')

        # current player
        self.curPlayerLabel = tk.Label(self, text="The current player is Player 1", font=("Calibri", 12, "bold"), bg=bg_1, fg=fg_1)
        ...
        for i in range(self.boardSize):  # row variable
            for j in range(self.boardSize):  # column variable
                self.buttons[i*self.boardSize+j] = tk.Button(self, width=5, height=1, command=lambda arg=(i,j): [self.set_row(arg[0]), self.set_col(arg[1])], font=("Calibri", 12, "bold"), bg=bg_2, fg=fg_1)
